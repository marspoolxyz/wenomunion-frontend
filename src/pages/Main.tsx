...
function Main({ venomConnect }: Props) {
  ...
  const [fundAction, setfundAction] =  useState<boolean | undefined>(true);
  ...
  const setupTokenWalletAddress = async (standalone: ProviderRpcClient, wallet: string): Promise<string | undefined> => {
    try {
      const contractAddress = new Address('0:65c3ff8fdd39c2487a9b0536c785ef8d528b1a6e8cefa9c2d03ddb1981255b6b'); // Our TokenRoot address in venom testnet
  ...
  const onTransferFund = async () => {
    setfundAction(true);
  };

  const onReceiveFund = async () => {
    setfundAction(false);
    console.log("TEST");
  };
  ...
  return (
    <div className="box">
      {address && (
        <header>
          <p onClick={onTransferFund}>
            Transfer Fund
          </p>
          <p onClick={onReceiveFund}>
            Receive Fund 
          </p>                    
          <p>{address}</p>
